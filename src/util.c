...
char *
util_format_curl_error(const char *method, long http_code, long curl_code,
...
char *
util_format_error(const char *method, const char *error_message, const char *file, int line) {
    JSON_Value *root_value = create_error(method, error_message, file, line);
...
bool util_starts_with(const char *a, const char *b) {
    if (XSTRNCMP(a, b, XSTRLEN(b)) == 0) return 1;
    return 0;
}
