...
static void _check_version(const char *version, const int line) {
    ...
    ASSERT_TRUE_MESSAGE(XSTRSTR(version, "libcertifier "
                                CERTIFIER_VERSION) == version,
                        "certifier version '%s' not in expected format",
                        version);
}
...
static inline void _check_crt(const char *b64_token, const char *type, const int line) {
    ...
    ASSERT_TRUE_MESSAGE(XSTRSTR(decoded, token_needle) != NULL,
                        "Could not find token in wrapper '%s' at line %d",
                        decoded,
                        line);
    free(decoded);
}
...
int run_easy_api_tests(void) {
    struct CMUnitTest tests[] = {
            CREATE_TEST(test_api_easy),
            CREATE_TEST(test_api_easy_create_tokens),
