		...
		if (dir != NULL)
		{
			...
			while ((entry = readdir(dir)) != NULL)
			{
				...
				if (strncmp(fileExtension, "p12", strlen("p12")) == 0)
				{
					...
					strncpy(s_list_of_certs[list_of_certs_idx] + offset, "/", 2); // 2 to avoid warning about null character truncation
	...
	for (size_t list_of_certs_idx = 0; *s_list_of_certs[list_of_certs_idx] != 0; ++list_of_certs_idx)
	{
		...
		if (ret < 0) {
			syslog(LOG_ERR, "Can not write to log stream: %s, error: %s",
				(log_stream == stdout) ? "stdout" : "unknown_file_name", strerror(errno));
			break;
		}
    ...
    for (size_t list_of_certs_idx = 0; *s_list_of_certs[list_of_certs_idx] != 0; ++list_of_certs_idx)
    {
		...
		switch (rc) {
			...
			case CERTIFIER_ERR_REGISTRATION_STATUS_CERT_EXPIRED_1:
				syslog(LOG_INFO, "Certificate from file %s status: Not yet valid. Not renewing it.", s_list_of_certs[list_of_certs_idx]);
				continue;
			case 0:
...
int main(int argc, char *argv[])
{
    ...
    if (cert_dir_list != NULL) free(cert_dir_list);
